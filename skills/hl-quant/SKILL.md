...
口径用**标准下行半偏差**（MAR=0）：分母是全部周期上 `min(r,0)²` 的均方根，而不是「只取负收益子集再算样本标准差」——后者会丢掉均值偏移、且在负收益样本很少时方差极大，导致 score 不稳。

成交时点同样固定为**无未来函数**：信号只用截至上一根已完成 bar 的信息，成交发生在下一根 bar 的开盘。「用当根收盘决策又按当根收盘成交」属于同 bar 前视，会系统性高估 score，必须避免。
